...
type BillingStepProps = {
  ...
  setBillingId: Dispatch<SetStateAction<string | null>>;
  handleNext: Function;
};
function BillingStep({
  ...
  useEffect(() => {
    if (addresses.length) {
      ...
      setBillingId(defaultAddress ? defaultAddress._id : addresses[0]._id);
    }
  }, [addresses]);

  // address handle change
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setBillingId((event.target as HTMLInputElement).value);
  };
    ...
    <>
      ...
      {billingId ? (
        <FormControl fullWidth>
          ...
          <RadioGroup value={billingId} onChange={handleChange}>
